...
- create and register `ADMIN_TOKEN` as a fine-grained PAT with `Repository permissions -> Administration -> Read and write`
...
- `branch-protection.main.verify-lite-noreview.json`
  - recommended preset when native PR review protection should be disabled entirely
...
- the current `main` baseline therefore keeps GitHub-native review protection enabled while using `0` required approvals
...
- `ADMIN_TOKEN`（Fine-grained PAT の `Repository permissions -> Administration -> Read and write` を付与）を作成して登録する
...
- `branch-protection.main.verify-lite-noreview.json`
  - GitHub native の PR review protection を完全に外したい場合の推奨 preset
...
- 現在の `main` baseline は、required approvals を `0` にしつつ GitHub native の review protection 自体は有効のまま維持している
