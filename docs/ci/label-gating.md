...
- `enforce-assurance`: make claim-level assurance summary enforcement blocking; recommended only for `risk:high` or critical core changes
...
### Claim-level assurance policy

Claim-level assurance follows the same opt-in posture as other heavy controls.

- Default: unresolved claims are report-only or warning evidence, but the summary must keep counts visible.
- `risk:high`: missing required lanes or unresolved high-risk claims require human review according to `policy/risk-policy.yml`.
- `enforce-assurance`: strict assurance enforcement can block missing required lanes, missing evidence, failed evidence, expired waivers, and unresolved claims.
- Waivers must carry owner, reason, expiry, affected claim, and evidence link; missing or expired waiver metadata is blocking in strict modes.
- Review surfaces should link normalized summaries (`claim-evidence-manifest/v1`, `claim-level-summary/v1`, `policy-decision/v1`) before raw logs.

The policy escalation matrix is defined in `policy/risk-policy.yml` under
`assurance_escalation`. Use it to decide whether a finding stays report-only,
requires manual approval, or becomes blocking.

| Condition | Finding classes | Decision | Required operator action |
| --- | --- | --- | --- |
| Ordinary `risk:low` PR without strict assurance or critical-core boundary | Missing required lane, missing evidence, Boundary Map drift, waiver metadata gap, unresolved claim | report-only | Keep counts and source artifact paths visible in summaries. Do not add heavy lanes solely because a producer emitted a finding. |
| `risk:high` | Same classes | manual approval | Require the configured human approval count, required policy labels, plan artifact, and explicit reviewer disposition. Findings must not be treated as satisfied/proved. |
| `enforce-assurance` | Missing required lane, missing evidence, failed evidence, expired/orphan waiver, unresolved claim | block | Run strict assurance after Verify Lite artifacts are available. Fix evidence, provide a valid waiver, or remove the strict label. |
| Critical-core boundary or explicit assurance profile | Profile-required lane/evidence, Boundary Map drift for critical slice, waiver metadata gap | manual approval or block | Follow the assurance profile / critical-core policy. Block only when the profile selects strict enforcement; otherwise require reviewer disposition. |
...
- `enforce-assurance`: claim-level assurance summary enforcement を blocking にする。`risk:high` または critical core 変更のみ推奨
...
Claim-level assurance は他の heavy control と同じ opt-in 姿勢に従います。

- 既定: unresolved claim は report-only または warning evidence ですが、summary は count を隠しません。
- `risk:high`: required lane 不足または unresolved high-risk claim は `policy/risk-policy.yml` に従い human review を要求します。
- `enforce-assurance`: strict assurance enforcement により、required lane 不足、evidence 不足、failed evidence、期限切れ waiver、unresolved claim を blocking にできます。
- waiver は owner、reason、expiry、affected claim、evidence link を持つ必要があります。不足または期限切れの waiver metadata は strict mode で blocking です。
- review surface は raw log より先に normalized summary（`claim-evidence-manifest/v1`、`claim-level-summary/v1`、`policy-decision/v1`）を link します。

policy escalation matrix は `policy/risk-policy.yml` の
`assurance_escalation` に定義します。finding を report-only に留めるか、
manual approval に上げるか、blocking にするかはこの表で判断します。

| 条件 | Finding class | 判定 | Operator action |
| --- | --- | --- | --- |
| strict assurance や critical-core boundary のない通常 `risk:low` PR | required lane 不足、evidence 不足、Boundary Map drift、waiver metadata gap、unresolved claim | report-only | summary に count と source artifact path を残す。producer が finding を出しただけで heavy lane を追加しない。 |
| `risk:high` | 同上 | manual approval | 設定済み human approval 数、required policy labels、plan artifact、reviewer disposition を要求する。finding を satisfied/proved として扱わない。 |
| `enforce-assurance` | required lane 不足、evidence 不足、failed evidence、期限切れ/孤立 waiver、unresolved claim | block | Verify Lite artifact が揃った後に strict assurance を実行する。evidence 追加、valid waiver、または strict label 除去で解消する。 |
| critical-core boundary または明示的 assurance profile | profile が要求する lane/evidence、critical slice の Boundary Map drift、waiver metadata gap | manual approval または block | assurance profile / critical-core policy に従う。profile が strict enforcement を選んだ場合だけ block し、それ以外は reviewer disposition を要求する。 |
