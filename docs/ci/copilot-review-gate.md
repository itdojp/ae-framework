...
- 補助トリガー: `.github/workflows/agent-commands.yml` の `issue_comment(created)`  
...
- auto-fix 直後の再評価は `agent-commands` 経由の dispatch で行われます。コメントが `edited` のみで `created` でない場合は自動再評価されないため、必要に応じて手動 rerun してください。
