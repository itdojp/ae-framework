...
- non-strict `missing required evidence` is `report-only` when WARN is allowed, but becomes `blocking` when strict validation is used or auto-merge disallows WARN
- evidence that is not expected for the current risk/profile should be modeled as not required instead of emitted as a missing required evidence warning
- stale generated artifacts or changed-file inputs must be regenerated for the current PR head before interpreting `files=0` or missing-evidence counts
- the PR automation category source is `docs/ci/pr-automation.md#42-pr-warning-category-semantics`
...
Validation warning の解釈:
- 非strictの `missing required evidence` は、WARN許可時は `report-only`、strict validation または auto-merge の WARN 不許可時は `blocking` として扱います。
- 現在の risk / profile で期待しない evidence は、missing required evidence warning として出さず、required 対象外として表現します。
- 古い生成artifactや changed-file 入力に基づく `files=0` / missing-evidence count は、現在のPR headで再生成してから解釈します。
- PR automation全体の category 定義は `docs/ci/pr-automation.md#42-pr-warning-category-semantics` を正とします。
