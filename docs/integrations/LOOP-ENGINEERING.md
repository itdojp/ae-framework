...
5. Paste `reviewSurface.body` or the Markdown summary into the PR/Issue when human review is needed.
6. Do not treat the loop summary as approval. Required checks, reviews, and repository policy remain authoritative.
...
5. human review が必要な場合は、`reviewSurface.body` または Markdown summary を PR / Issue に貼る。
6. loop summary を承認として扱わない。required checks、reviews、repository policy が引き続き authoritative evidence である。
