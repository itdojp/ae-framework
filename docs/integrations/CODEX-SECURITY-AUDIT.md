...
This runbook describes how to implement and operate the ae-framework **Security Assurance Lane** from Codex without relying on external LLM calls in the MVP path.
...
specification / design / audit scope
  ...
  -> security-finding/v1 candidates
...
| 11 | #3292 | Codex audit prompt pack | `ae security audit-prompt`, `security-audit-prompt-pack/v1`, deterministic Markdown prompts |
...
pnpm -s run security:audit-prompt -- \
  --tasks "$SECURITY_OUT/security-audit-tasks.json" \
  --code-map "$SECURITY_OUT/security-code-map.json" \
  --claims "$SECURITY_OUT/security-claims.json" \
  --out "$SECURITY_OUT/codex-audit-prompts" \
  --generated-at "$GENERATED_AT"
...
この runbook は、Codex から ae-framework の **Security Assurance Lane** を実装・運用するための手順をまとめたものです。MVP の実行経路では外部 LLM 呼び出しを前提にしません。
