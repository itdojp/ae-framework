...
New-Item -ItemType Directory -Force -Path (Join-Path $Work ".codex-local/tasks") | Out-Null
$issueJson = gh issue view $Issue --repo $Repo --json title,body,url | ConvertFrom-Json
$task = "# $($issueJson.title)`n`nIssue: $($issueJson.url)`n`n$($issueJson.body)"
Set-Content -Path (Join-Path $Work ".codex-local/tasks/issue-$Issue.md") -Value $task -NoNewline -Encoding utf8
```
