...
- Stdio bridge (no MCP): `pnpm run codex:spec:stdio`
  ...
  - Write-producing actions require trusted approval and are constrained to `artifacts/spec-synthesis` by default.
  - Codegen materialization also invokes the shared high-impact approval scope `high-impact:codegen-materialize`; untrusted workspaces/refs are forced to dry-run by the child codegen policy.
...
```bash
# Strict compile → AE‑IR (artifacts/spec-synthesis/ae-ir.json)
printf '%s\n' '{"action":"compile","approval":{"approved":true,"scope":"codex-spec-stdio"},"args":{"inputPath":"spec/feature.ae-spec.md","outputPath":"artifacts/spec-synthesis/ae-ir.json","relaxed":false}}' | AE_CODEX_SPEC_STDIO_TRUSTED_APPROVAL=1 pnpm run codex:spec:stdio
```
...
Security boundary:
- `validate` is read-only, but a cold checkout may need trusted context if the bridge must auto-build the local spec compiler.
- `compile` with `outputPath` and `codegen` require trusted approval (`AE_CODEX_SPEC_STDIO_TRUSTED_CONTEXT=1`, or `AE_CODEX_SPEC_STDIO_TRUSTED_APPROVAL=1` plus `approval.approved=true`). For codegen, use the narrow request scope `codex-spec-stdio-codegen` or a documented umbrella scope.
- The bridge materializes code through `codegen generate --apply --approval-scope high-impact:codegen-materialize`; the child CLI still checks trusted workspace/ref and falls back to dry-run when the checkout is untrusted.
- Request paths must be workspace-relative; absolute paths, `..` / `.` path segments, backslashes, and `.git` targets are rejected.
- Writes are limited to `artifacts/spec-synthesis` unless a trusted runner explicitly sets `AE_CODEX_SPEC_STDIO_ALLOW_WORKSPACE_WRITES=1`.
...
- `ae_spec_compile` with `{ inputPath, outputPath: "artifacts/spec-synthesis/ae-ir.json", relaxed: false }`
- `ae_spec_codegen` with `{ irPath: "artifacts/spec-synthesis/ae-ir.json", targets: ["typescript","api","database"], approvalScope: "high-impact:codegen-materialize" }`
...
- Final: `spec/*.ae-spec.md`, `.ae/ae-ir.json`, `generated/*` when materialized with approval; otherwise codegen remains a dry-run preview.
