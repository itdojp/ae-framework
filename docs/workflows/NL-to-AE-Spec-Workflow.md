...
- `compile` with `outputPath` and `codegen` require trusted approval (`AE_CODEX_SPEC_STDIO_TRUSTED_CONTEXT=1`, or `AE_CODEX_SPEC_STDIO_TRUSTED_APPROVAL=1` plus `approval.approved=true`).
...
- `ae_spec_codegen` with `{ irPath: "artifacts/spec-synthesis/ae-ir.json", targets: ["typescript","api","database"] }`
