...
- KvOnce のトレース検証 (`scripts/trace/collect-sample-otlp.mjs` → `scripts/trace/convert-otlp-kvonce.mjs` → `scripts/trace/run-kvonce-conformance.sh`) を CI に組み込み、サンプル OTLP ログの収集 / 変換を自動化
...
node scripts/trace/collect-sample-otlp.mjs
