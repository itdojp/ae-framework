...
- No external repository pilot was included; the follow-up path is now defined
  in `docs/guides/external-pilot-onboarding.md` and should remain report-only
  until consent, redaction, and publication boundaries are recorded.
...
4. Run a consent-safe external-repository pilot with `docs/guides/external-pilot-onboarding.md` before making adoption-speed claims, and feed only approved aggregate/redacted data into ACP-097.
...
- 外部 repository pilot は未実施。後続 path は `docs/guides/external-pilot-onboarding.md` に定義し、consent、redaction、publication boundary を記録するまでは report-only に留める。
...
`docs/guides/external-pilot-onboarding.md` に従い、承認済みの aggregate / redacted data だけを ACP-097 に接続する。
