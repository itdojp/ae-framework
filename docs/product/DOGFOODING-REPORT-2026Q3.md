...
### 7. Req2run adoption-evidence template

This historical dogfooding report predates the req2run collector, so it does not
retroactively claim requirement-to-runnable speed or external adoption impact.
Future dogfooding rows may add the report-only fields defined in
`docs/product/REQ2RUN-METRICS.md`:

| Field | Recording rule |
| --- | --- |
| Observation id | Link or redacted id for the requirement / Issue / task. |
| Reference-flow branch | Baseline, structured assurance, Spec Kit import, Spec & Verification Kit, high-assurance selected claim, or report-only loop branch. |
| Req2run report | Path to `artifacts/metrics/req2run-metrics.json` or an approved redacted copy. |
| Core metrics | `time_to_first_runnable_verification_minutes`, `spec_task_evidence_coverage`, `deterministic_replay_pass_rate`, `manual_intervention_count`, and `evidence_review_completeness`; use `not_collected` rather than zero when absent. |
| Claim boundary | State that values measure ae-framework workflow effectiveness, not agent vendor quality. |
| Publication boundary | Record `synthetic`, `approved`, `aggregate only`, `private`, or `not approved for publication`. |

### 8. Limitations and residual risk

- No live external repository pilot was included; the follow-up path is now
  defined in `docs/guides/external-pilot-onboarding.md`, and the current
  ACP-097 status is documented in `docs/product/PILOT-REPORT-2026Q3-01.md`
  as `dry-run only` until consent, redaction, and publication boundaries are
  recorded for live PRs.
...
### 9. Launch-kit summary paragraph
...
4. Replace the `dry-run only` ACP-097 report in `docs/product/PILOT-REPORT-2026Q3-01.md` with maintainer-approved aggregate/redacted live pilot data before making adoption-speed claims.
...
- live external repository pilot は未実施。後続 path は `docs/guides/external-pilot-onboarding.md` に定義し、現在の ACP-097 status は `docs/product/PILOT-REPORT-2026Q3-01.md` で `dry-run only` として記録する。Consent、redaction、publication boundary を記録するまでは report-only に留める。
...
### 6. Req2run adoption evidence template

この historical dogfooding report は req2run collector 追加前のものなので、requirement-to-runnable
speed や external adoption impact を後付けで主張しません。今後の dogfooding row では
`docs/product/REQ2RUN-METRICS.md` の report-only fields を追加できます。

| Field | Recording rule |
| --- | --- |
| Observation id | requirement / Issue / task の link または redacted id。 |
| Reference-flow branch | Baseline、structured assurance、Spec Kit import、Spec & Verification Kit、high-assurance selected claim、report-only loop branch。 |
| Req2run report | `artifacts/metrics/req2run-metrics.json` または承認済み redacted copy。 |
| Core metrics | 5つの req2run metric。未収集値は 0 ではなく `not_collected`。 |
| Claim boundary | ae-framework workflow effectiveness を測り、agent vendor quality を測らないと明記する。 |
| Publication boundary | `synthetic` / `approved` / `aggregate only` / `private` / `not approved for publication` を記録する。 |

### 7. Launch kit 用 summary
...
speed や agent vendor quality の benchmark ではない。外部 pilot で使う場合は
`docs/guides/external-pilot-onboarding.md` と `docs/product/PILOT-REPORT-2026Q3-01.md` に従い、承認済みの aggregate / redacted data だけを ACP-097 に接続する。
