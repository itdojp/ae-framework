...
本資料での前提は、ae-framework を「コード生成ツール」ではなく、**spec / verification / evidence / policy gate を束ねる agent-neutral assurance control plane** として捉えることです。codegen は producer の一つであり、SSOT と判断面の中心は contract と artifact にあります。
