...
The current design assumption is that ae-framework is not merely a code generator. It is an agent-neutral assurance control plane that coordinates specifications, verification, evidence, and policy decisions. Code generation is only one producer among several, and the system of record stays in contracts and artifacts. Context Pack is the design SSOT/input contract for the specification, boundary, acceptance, and traceability inputs that agents and humans use before producing code.
...
| Structured assurance | services that need specification-linked evidence | Context Pack as design SSOT, property/MBT/conformance, change evidence | boundary-map report, conformance summary, assurance summary, change package, hook-feedback |
...
For the BYO-agent adoption sequence and producer boundary checklist, use
...
| --- | --- | --- | --- | --- |
| Web API / BFF | spec drift and unstable PR quality | requirement markdown, API spec, minimum tests | `pnpm run verify:lite` | `artifacts/verify-lite/verify-lite-run-summary.json` (local command), `artifacts/report-envelope.json` (CI: generated by `scripts/trace/create-report-envelope.mjs`) |
| Event-driven domains (inventory, ordering, payment) | invariant violations and event-order inconsistency | selected event trace, sample events, rule definitions | `pnpm run conformance:verify:selected-trace` | `artifacts/hermetic-reports/conformance/selected-trace-summary.json` |
...
| Context Pack / Boundary Map | `spec/context-pack/**/*.{yml,yaml,json}`, `spec/context-pack/boundary-map.json` | fixes the design SSOT and implementation slice boundaries shared by agents and humans |
...
4. Add domain-specific lanes
   ...
   - Conformance: `pnpm run conformance:verify:selected-trace`
...
本資料での前提は、ae-framework を「コード生成ツール」ではなく、**spec / verification / evidence / policy gate を束ねる agent-neutral assurance control plane** として捉えることです。codegen は producer の一つであり、SSOT と判断面の中心は contract と artifact にあります。Context Pack は、agent と human がコード生成前に参照する仕様・境界・acceptance・traceability の design SSOT / input contract です。
...
| Structured assurance | 仕様起点の整合性が必要なサービス | design SSOTとしてのContext Pack, property/MBT/conformance, change evidence | boundary-map report, conformance summary, assurance summary, change package, hook-feedback |
...
BYO-agent 環境での導入順と producer boundary checklist は
`docs/guides/byo-agent-assurance-onboarding.md` を参照してください。
...
| --- | --- | --- | --- | --- |
| Web API / BFF | 仕様と実装の乖離、PR品質のばらつき | 要件Markdown、API仕様、最小テスト | `pnpm run verify:lite` | `artifacts/verify-lite/verify-lite-run-summary.json`（ローカル実行）、`artifacts/report-envelope.json`（CI: `scripts/trace/create-report-envelope.mjs` により生成） |
| イベント駆動（在庫/注文/決済） | 不変条件違反、イベント順序不整合 | selected event trace、サンプルイベント、ルール定義 | `pnpm run conformance:verify:selected-trace` | `artifacts/hermetic-reports/conformance/selected-trace-summary.json` |
...
| Context Pack / Boundary Map | `spec/context-pack/**/*.{yml,yaml,json}`, `spec/context-pack/boundary-map.json` | agent / human が共有する design SSOT と実装slice境界を固定する |
