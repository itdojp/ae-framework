...
| Event-driven domains (inventory, ordering, payment) | invariant violations and event-order inconsistency | sample events, rule definitions, traces | `pnpm run conformance:verify:sample` | `artifacts/hermetic-reports/conformance/sample-results.json` |
...
| イベント駆動（在庫/注文/決済） | 不変条件違反、イベント順序不整合 | サンプルイベント、ルール定義、トレース | `pnpm run conformance:verify:sample` | `artifacts/hermetic-reports/conformance/sample-results.json` |
