...
| --- | --- | --- | --- | --- | --- |
| `review_decision_time` | How long did it take from PR review start to a defensible reviewer decision? | PR review timeline, review thread state, PR summary, optional manual timing note. | Record minutes from first human-review-ready event to first reviewer decision (`approve`, `request changes`, `defer`, or `block`). | Lower time can indicate a clearer review surface when decision quality is not worse. | Timing is sensitive to reviewer availability, timezone, queueing, and PR size. Do not claim causality without a controlled baseline. |
...
| `policy_gate_false_negative_count` | How many policy passes missed a finding later judged policy-relevant? | Post-review findings, incident/defect notes, manual operator annotation, policy decision history. | Count policy-relevant misses discovered after a pass and classify root cause. | Tracks policy recall and missed-risk risk. | Usually sparse and delayed; absence of observations is not proof of zero false negatives. |
...
metrics when a machine-readable report is available.
...
| Metric | Reviewer question | Source artifact / source | Collection method | Interpretation | Limitation |
| --- | --- | --- | --- | --- | --- |
| `review_decision_time` | PR review 開始から defensible reviewer decision までにどれくらいかかったか。 | PR review timeline、review thread state、PR summary、optional manual timing note。 | human-review-ready event から最初の reviewer decision（`approve` / `request changes` / `defer` / `block`）までの分数を記録する。 | decision quality が悪化していない場合、短縮は review surface の明確化を示し得る。 | reviewer availability、timezone、queue、PR size の影響が大きい。Controlled baseline なしで因果を主張しない。 |
...
| `time_to_merge` | open / ready-for-review から merge までどれくらいかかったか。 | PR timeline、merge timestamp、review and CI state。 | ready-for-review から merge までの分数を測り、可能なら waiting-for-review、waiting-for-CI、fixing-review、blocked segment に分ける。 | segment付きなら adoption / throughput metric として使える。 | queue、reviewer availability、branch protection、release window の影響が大きい。 |
...
`docs/ci/agent-pr-assurance-metrics.md` と `schema/agentic-metrics.schema.json` の optional `agentPrAssurance` extension は、PR assurance metrics の低レベルな report-only contract です。この product 文書はより広い product-effectiveness vocabulary を定義し、machine-readable report がある場合はそれらの低レベルmetricへ対応付けます。
