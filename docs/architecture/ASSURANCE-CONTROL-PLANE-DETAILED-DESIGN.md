...
- This design does not introduce `not-applicable` as a current primary emitted evidence state for `claim-evidence-manifest/v1`. `change-package/v2` may preserve `not-applicable` as a package/release outcome state, and `claim-level-summary/v1` may project it for PR/release review, but neither surface redefines primary evidence states without explicit migration.
...
Current primary evidence states and package representations are:
...
`not-applicable` is intentionally not part of the current `claim-evidence-manifest/v1` primary evidence state set. `change-package/v2` can preserve it as an explicit package/release outcome state, and preview `claim-level-summary/v1` can project it for PR/release review and migration testing. Producers must not project that package/projection state back into `claim-evidence-manifest/v1` or agent PR metrics until promotion is explicit.
...
7. Do not emit `not-applicable` as a primary evidence state until the schema, fixtures, docs, producer, policy-gate, and summary consumers accept that promotion. `change-package/v2` may continue to carry it as a package/release outcome state.
...
- 現行 primary evidence contract が emit できる state は `proved`、`model-checked`、`tested`、`runtime-mitigated`、`waived`、`unresolved` である。
- `not-applicable` は `claim-evidence-manifest/v1` の primary evidence state ではない。`change-package/v2` では package / release outcome state として保持でき、`claim-level-summary/v1` では PR / release projection として表示できるが、primary evidence state へ戻すには明示的な schema / policy migration が必要である。
