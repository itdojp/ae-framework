...
- Formal: `artifacts/summary/combined.json.formal` を優先し、current renderer の fallback として legacy `formal/summary.json`、次に `artifacts/hermetic-reports/formal/summary.json` を参照します。`artifacts/formal/formal-aggregate.json` は aggregate detail 用です
...
- Read from normalized JSON artifacts:
  ...
  - `artifacts/formal/formal-aggregate.json` (optional aggregate detail)
  - legacy `formal/summary.json` or `artifacts/hermetic-reports/formal/summary.json` (optional fallback)
...
- Current formal fallback order is `combined.json.formal` → legacy `formal/summary.json` → `artifacts/hermetic-reports/formal/summary.json`; `artifacts/formal/formal-aggregate.json` is supplementary aggregate detail input.
