...
If these variables are unset, the workflow still runs in report-only mode. For enforcement (`enforce-perf` / `enforce-lh`), you must set `PERF_DEFAULT_THRESHOLD` / `LH_DEFAULT_THRESHOLD` or provide an override label; otherwise the enforcement step treats the threshold as `0` rather than the documented defaults, and the `label > repo var > default` resolution does not apply in that case.
...
- perf:
  - one of `.score`, `.performance`, or `.metrics.score` must be present for JSON shape validation
  - note: `.lighthouse.performance` is accepted by `summarize-perf`, but the shape check still emits a warning unless one of the fields above is also present
- lighthouse:
...
Add `run-adapters` on the PR and then trigger a new `pull_request` event, for example by pushing an additional commit or reopening the PR, so the workflow posts the summary comment. Add `enforce-a11y` only when you want the threshold to block the PR.
...
これらが未設定でも workflow は report-only で動作する（この場合の実効値の解決順は `label > repo var > default`）。一方で `enforce-*` による enforcement を有効化する場合は、`PERF_DEFAULT_THRESHOLD` / `LH_DEFAULT_THRESHOLD` の設定が必須であり、これらが未設定かつ override label が無い場合はしきい値が `0` として扱われる。
...
- perf:
  ...
  - `summarize-perf` は `.lighthouse.performance` も読み取れるが、shape check の jq では上記 3 つのキーのみを確認するため、`.lighthouse.performance` しか無い場合は warning となる
- lighthouse:
...
その後、PR に `run-adapters` を付け、追加 commit の push や PR の reopen などで新しい `pull_request` event を発生させて workflow の要約コメントを確認する。しきい値を block させたい場合だけ `enforce-a11y` を併用する。
