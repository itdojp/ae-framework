...
`change-package/v2.claims[].status` is a package / release-decision outcome field. It may retain `satisfied`, `failed`, and `not-applicable` package states for reviewable release packages, but those package states do not redefine `claim-evidence-manifest/v1` primary evidence states.

`agentPrAssurance.metrics.required_lane_compliance.notApplicable` is separate from the claim-status vocabulary. It is only a metric-level denominator state for "no required lanes"; producers must not emit `not-applicable` as a primary claim state unless the primary evidence schema and migration policy explicitly allow it.
...
`change-package/v2.claims[].status` は package / release-decision outcome field です。review 可能な release package のために `satisfied`、`failed`、`not-applicable` などの package state を保持できますが、それらは `claim-evidence-manifest/v1` の primary evidence state を再定義しません。

`agentPrAssurance.metrics.required_lane_compliance.notApplicable` は claim status vocabulary とは別の概念です。「required lane がない」ことを表す metric-level denominator state に限られ、primary evidence schema と migration policy が明示的に許可するまで、producer は `not-applicable` を primary claim state として emit してはいけません。
