...
`agentPrAssurance.metrics.required_lane_compliance.notApplicable` は claim status vocabulary とは別の概念です。「required lane がない」ことを表す metric-level denominator state に限られ、claim-level schema と migration policy が明示的に許可するまで、producer は `not-applicable` を primary claim state として emit してはいけません。
