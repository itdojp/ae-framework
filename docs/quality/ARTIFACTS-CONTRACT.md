...
| `artifacts/ci/policy-gate-summary.json` | `policy-gate.yml` を実行した場合 | policy gate 判定の機械可読成果物（schemaVersion: `policy-gate-summary/v1`, contractId: `policy-gate-summary.v1`）。`claim-evidence-manifest/v1` が存在する場合は report-only の waiver-aware assurance decision section を含む |
| `artifacts/ci/policy-gate-summary.md` | `policy-gate.yml` を実行した場合 | policy gate 判定の Markdown 要約。waiver-aware assurance section は `pass` と `waived` を混同せず、`report-only` / `block` も区別して表示する |
