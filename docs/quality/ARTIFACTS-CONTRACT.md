...
- `artifacts/assurance/claim-evidence-manifest.json` is a claim-level evidence sidecar contract; PR #3243 introduces its schema and fixture first, with generator/rendering follow-ups staged separately.
...
- `artifacts/assurance/claim-evidence-manifest.json` は claim 単位の evidence sidecar contract です。PR #3243 では schema / fixture を先に追加し、generator / renderer は後続 PR で接続します。
...
| `artifacts/assurance/claim-evidence-manifest.json` | `claim-evidence-manifest/v1` generator を実行した場合（PR #3243 後続で接続予定） | claim 単位の statement / criticality / targetLevel / achievedLevel / status / evidenceRefs / missingEvidenceRefs / waiverRefs を保持する sidecar contract（schema: `schema/claim-evidence-manifest.schema.json`）。PR #3243 の初期 PR では fixture と contract validation を先に固定する |
