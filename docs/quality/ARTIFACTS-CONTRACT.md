...
| `artifacts/context-pack/context-pack-boundary-map-report.json` | `pnpm run context-pack:verify-boundary-map` を実行した場合 | boundary-map sidecar と Context Pack の参照整合、missing slice、cycle を検証する report-only JSON |
| `artifacts/context-pack/context-pack-boundary-map-report.md` | `pnpm run context-pack:verify-boundary-map` を実行した場合 | boundary-map 検証の Markdown 要約。manual review / onboarding 用 |
