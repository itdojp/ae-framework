...
- `artifacts/` には runtime output / committed contract artifact / reference snapshot / archive が混在します。commit 対象の可否は repo layout policy を優先し、schema/consumer 契約は本書を優先します。
...
| `artifacts/bench.json` | `pnpm exec tsx src/cli.ts bench` を実行した場合 | TS baseline 計測成果物（schema: `schema/benchmark-report.schema.json`）。現状は legacy compatibility shim 経由 |
