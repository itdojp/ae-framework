...
If `formal/summary.json` is kept as a legacy compatibility input, applying equivalent metadata is still recommended. Route status for this fallback is tracked in `docs/reference/ASSURANCE-CANONICAL-ROUTES.md`.
...
- assurance / scorecard / handoff
  ...
  - `artifacts/temporal/*/temporal-run-summary.json`
...
- `artifacts/temporal/*/temporal-run-summary.json`
  - produced by the optional `examples/temporal-workflow-adapter` PoC
  - records Temporal Workflow execution metadata, signal summaries, activity artifact refs, and event-history pointers without replacing the existing assurance contracts
- `artifacts/quality/quality-scorecard.{json,md}`
...
- legacy compatibility input として `formal/summary.json` を扱う場合も、同等の `metadata` を付与する運用を推奨します。この fallback の導線状態は `docs/reference/ASSURANCE-CANONICAL-ROUTES.md` で追跡します。
...
| `artifacts/temporal/*/temporal-run-summary.json` | optional Temporal Workflow adapter PoC を実行した場合 | Temporal Workflow の namespace / Workflow ID / Run ID / task queue / signal / activity artifact refs / event-history pointer を保持する adapter sidecar contract（schema: `schema/temporal-run-summary.schema.json`）。既存 assurance / policy artifact は source of truth のまま維持する |
...
| `artifacts/ci/policy-gate-summary.json` | `policy-gate.yml` を実行した場合 | policy gate 判定の機械可読成果物（schemaVersion: `policy-gate-summary/v1`, contractId: `policy-gate-summary.v1`）。`claim-evidence-manifest/v1` が存在する場合は report-only の waiver-aware assurance decision section を含む。producer / assurance-summary 由来の `agentAssuranceFindings` は count / severity / source artifact path 付きの report-only evidence として保持する |
| `artifacts/ci/policy-gate-summary.md` | `policy-gate.yml` を実行した場合 | policy gate 判定の Markdown 要約。waiver-aware assurance section は `pass` と `waived` を混同せず、`report-only` / `block` も区別して表示する。agent assurance findings は通常PRをblockせず、reviewer向けの report-only finding として表示する |
...
| `artifacts/context-pack/boundary-map-summary.json` | `pnpm run context-pack:verify-boundary-map` を実行した場合 | PR evidence 用の summary-first JSON（schema: `schema/context-pack-boundary-map-summary.schema.json`）。`status=ok/drift/skipped/unresolved` と expected owner / observed dependency を保持し、drift を proof failure ではなく design-boundary evidence gap として扱う |
| `artifacts/context-pack/boundary-map-summary.md` | `pnpm run context-pack:verify-boundary-map` を実行した場合 | PR / operator review 用の Markdown summary。high-risk / enforce-assurance / critical-core boundary で block 昇格し得る drift の転記元 |
