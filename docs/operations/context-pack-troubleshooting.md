...
### agent差分とdesign SSOTの衝突
Codex / Copilot / Claude Code などが生成した差分が Context Pack と矛盾する場合、agent出力ではなく Context Pack 側を design SSOT として扱います。

- Issue本文、`AGENTS.md`、Context Pack、boundary map、acceptance tests の順に入力を再確認する。
- Context Pack が現行設計を表しているなら、差分を差し戻すか修正する。
- Context Pack が古いなら、実装変更とは別に Context Pack 更新と validation を含める。
- Baselineでは `context-pack:validate` と `context-pack:verify-boundary-map` を優先し、Structured assurance / High-assurance critical core の追加検証はriskやcriticalityに応じて選択する。
...
### Agent diff vs. design SSOT conflicts
When a diff generated by Codex, Copilot, Claude Code, or another producer conflicts with the Context Pack, treat the Context Pack as the design SSOT rather than trusting the agent output.

- Re-read inputs in this order: GitHub Issue, `AGENTS.md`, Context Pack, Boundary Map, then acceptance tests.
- If the Context Pack still represents the intended design, reject or revise the diff.
- If the Context Pack is stale, include the Context Pack update and validation separately from the implementation change.
- For Baseline, prioritize `context-pack:validate` and `context-pack:verify-boundary-map`; add Structured assurance or High-assurance critical core checks only when risk or criticality requires them.
