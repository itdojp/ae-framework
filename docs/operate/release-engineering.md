...
note evidence, successful external action-path resolution, and public external
consumer pass/block runtime smoke for both the immutable and moving action refs
for live Marketplace; and successful apply/fetch evidence with every desired
context for live branch protection. The Marketplace runtime contract also
...
Marketplaceを`live`にするにはlisting / release note / action pathに加え、source
repositoryとは異なるallowlisted public consumer repositoryでimmutable tagとmoving
`v1`のpass / block runtime smokeが必要です。両refのresolved commit一致、public
Actions run URL、`gate-result.json`、`assurance-summary.json`、
`policy-decision.json`、`review-surface.md`の存在をchecked-in contractで検証します。
Path resolution、source repository内test、local fixtureだけではこの条件を満たしません。
