...
- Append to `$GITHUB_OUTPUT` / `$GITHUB_ENV` using `printf` with quoting; do not use `echo` for these files.
  ...
  - Do not use deprecated commands (`::set-output`, `::add-path`); use `$GITHUB_OUTPUT` / `$GITHUB_ENV` instead
...
- `.github/workflows/formal-aggregate.yml` (Formal reports aggregate/comment)
 ...
 - `.github/workflows/nightly.yml` (scheduled nightly CI — non-blocking for PRs)
 - `.github/workflows/nightly-monitoring.yml` (scheduled monitoring — non-blocking for PRs)
...
- See also: `docs/quality/pr-summary.md` (summary comment structure and templates)
 - File output: `artifacts/summary/PR_SUMMARY.md`
...
- `.github/workflows/formal-aggregate.yml`（フォーマル集約/コメント）
 ...
 - `.github/workflows/nightly.yml`（夜間 CI: スケジュール実行。PR には非ブロッキング）
 - `.github/workflows/nightly-monitoring.yml`（夜間モニタリング: スケジュール実行。PR には非ブロッキング）
...
- `$GITHUB_OUTPUT` / `$GITHUB_ENV` への追記は `printf` + 適切なクォートを使用（`echo` は不可）。
  ...
  - 複数行の出力:
    - `printf '%s<<EOF\n%s\nEOF\n' 'summary' "$MULTILINE" >> "$GITHUB_OUTPUT"`
  - 非推奨コマンド（`::set-output`, `::add-path`）は使用しないでください。代わりに `$GITHUB_OUTPUT` / `$GITHUB_ENV` を使用
...
- Formal Aggregate コメント:
 ...
 - ファイル出力: `artifacts/summary/PR_SUMMARY.md`
