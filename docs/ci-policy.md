...
- `enforce-assurance`: in Verify Lite, fail the PR when `artifacts/assurance/assurance-summary.json` still reports warning claims, missing required lanes/evidence kinds, or open/unlinked counterexamples
...
- `enforce-assurance` ラベル時は、Verify Lite 内で `scripts/ci/enforce-assurance-summary.mjs` を実行し、warning claim・missing lane/evidence・open/unlinked counterexample を blocking 化する。
...
- Mutation auto-diff is advisory on PR/push entry runs because Stryker initial dry-runs can exceed the quick-run window before producing a report; the scheduled CI Extended lane remains strict so missing or failing mutation evidence is still surfaced as a maintenance signal.
...
- `enforce-assurance`: Verify Lite の assurance summary を strict に評価し、warning claim・missing lane/evidence・open/unlinked counterexample が残っている場合は失敗させる
...
- Mutation auto-diff は PR / push entry run では advisory 扱いです。Stryker の初期 dry-run が quick-run window を超え、レポート生成前に timeout することがあるためです。scheduled CI Extended lane は strict のまま維持し、mutation evidence の欠落や失敗を運用シグナルとして検出します。
