...
| What did the agent claim, and what evidence is missing? | `producer-normalization-summary/v1` and `assurance-summary/v1`. | `pnpm run producer-output:normalize -- --input <raw-producer-output.json>`; `pnpm -s run verify:assurance -- --producer-summary <summary.json>`. |
...
| agent は何を主張し、何のevidenceが不足しているか。 | `producer-normalization-summary/v1` と `assurance-summary/v1`。 | `pnpm run producer-output:normalize -- --input <raw-producer-output.json>`; `pnpm -s run verify:assurance -- --producer-summary <summary.json>`。 |
