...
sanitized dry run, or live mode only when the maintainer has approved read
access and the entry criteria in
`docs/product/LIVE-PILOT-ENTRY-CRITERIA.md` are satisfied. Before live
collection, list the target repository's actual required-check names; `gate`,
`policy-gate`, and `verify-lite` are only ae-framework defaults. Pass each
target check with the repeatable `--required-check` option so CI/rerun metrics
reflect the pilot repository rather than this repository.
...
- [ ] The entry criteria in `docs/product/LIVE-PILOT-ENTRY-CRITERIA.md` are
      satisfied for live collection, or this PR remains fixture/synthetic only.
...
Pilot PR ごとに report-only metrics collector を実行します。Sanitized dry run では fixture mode、maintainer が read access を承認し、かつ `docs/product/LIVE-PILOT-ENTRY-CRITERIA.md` の entry criteria を満たす場合だけ live mode を使います。Live collection 前に target repository の実際の required-check name を列挙します。`gate`、`policy-gate`、`verify-lite` は ae-framework の既定値にすぎません。Pilot repository の CI / rerun metrics になるよう、repeatable な `--required-check` で target check をそれぞれ指定します。
...
- [ ] Live collection の場合は `docs/product/LIVE-PILOT-ENTRY-CRITERIA.md` の
      entry criteria を満たしている。満たしていない場合、この PR は fixture / synthetic only とする。
