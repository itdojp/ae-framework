...
変更が Context Pack の制約と矛盾する場合は、実装差分を作る前に Context Pack 更新または要求修正を判断します。通常変更は最小Context Pack + `verify-lite` で開始し、traceability が必要な場合に Structured assurance、critical core のみ High-assurance lane へ昇格します。
...
If the requested change conflicts with Context Pack constraints, decide whether to update the Context Pack or revise the request before generating implementation changes. Start routine changes with a minimal Context Pack plus `verify-lite`, add Structured assurance when traceability is needed, and promote only critical core slices to the High-assurance lane.
