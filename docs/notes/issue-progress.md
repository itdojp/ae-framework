...
| #1005 | Test Stability Phase1 | 🚧 進行中 | Vitest workspace を導入し `test:unit` / `test:int` / `test:perf` が実際の suite を実行するよう修正。IntegrationTestOrchestrator にランナー lifecycle (setup/before/after/teardown) を組み込み、Hook 呼び出しのユニット検証と MaxListeners 警告の抑止を整備済み。UnifiedAgent 系の phase-state 副作用を環境変数で切り替え可能にし、integration CLI テストは一時ディレクトリへ chdir する形でリソースを隔離。AE_INTEGRATION_TRACE_HANDLES フラグで `why-is-node-running` を任意化しつつ共通クリーンアップ helper を導入、optimization/system suite も helper 経由で自動停止化。integration suite は `system-validation` / `test-orchestrator` を helper ベースへ移行済みで、phase-state tempDir と出力ディレクトリを自動清掃。Phase1 チェックリスト（integration fix / cleanup / timeout / retry）はすべて ✅ に更新済みで、次は広範なリトライ制御と TRACE_HANDLES 運用ドキュメント整備、残りの再試行ポリシー適用を進める。|
