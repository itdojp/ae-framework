...
| #1005 | Test Stability Phase1 | 🚧 進行中 | Phase1（integration cleanup / timeout / retry）は完了済み。Phase2 として `test:ci:lite` / `test:ci:extended` を導入し、Verify Lite を軽量化。`ci-extended` ワークフローを新設して `run-ci-extended` / `run-integration` / `run-property` / `run-mbt` / `run-mutation` ラベルで opt-in 実行を切替可能にした。引き続き Vitest workspace 移行や共通 helper（AE_INTEGRATION_TRACE_HANDLES 等）を維持しつつ、再実行時のキャッシュ活用と mutation dry-run の tempDir 整備が残課題。|
