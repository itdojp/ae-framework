...
| Trace Replay | `pnpm run trace:kvonce:replay` | 2025-10-05 | ✅ | KvOnce NDJSON サンプルを検証 → `pnpm run spec:kv-once:tlc` を実行し、`artifacts/hermetic-reports/trace/replay/kvonce-trace-replay.json` にまとめる。TLC 未導入の場合は `tool_not_available` を記録しつつ成功扱い。 |
