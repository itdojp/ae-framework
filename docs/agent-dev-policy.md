...
- **Required checks**：Branch protection で必須化されたステータスチェック（current main baseline は `verify-lite` / `policy-gate` / `gate`）。
...
## 基本方針（結論）
1. **ae-framework は GitHub Actions（`verify-lite` + `policy-gate` + `gate`）前提のため、原則 PR 単位で実行する。**
