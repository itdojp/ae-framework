...
| `artifacts/review/agent-assurance-demo/assurance-review.md` is missing | The demo did not complete or artifacts were cleaned. | Re-run `pnpm run demo:agent-assurance`; the command prints the reviewer surface path on success. |
...
| `artifacts/review/agent-assurance-demo/assurance-review.md` がない | demo が完了していないか artifacts が削除されています。 | `pnpm run demo:agent-assurance` を再実行します。成功時は reviewer surface path が出力されます。 |
