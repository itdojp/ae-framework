...
| Formal runner / model checker / proof tool | `pnpm run verify:formal`, `verify:tla`, `verify:csp`, `verify:lean` | proof/model-check result, counterexample, assumptions | tool output supports only the modeled/proved scope; it is not whole-product proof | formal summary, `assurance-summary/v1`, `claim-evidence-manifest/v1`, optional `change-package/v2` | formal summary generation, schema validation, assumption and scope review | counterexamples remain blocking evidence for affected claims; bounded/model assumptions must be recorded; waiver cannot relabel failed proof as `proved` |
...
| GitHub Copilot cloud agent PR | Copilot-created / Copilot-reviewed PR | PR diff、review body、inline comment、suggestion | Copilot comment は review input。対応・解決まで control-plane evidence ではない | review thread state、`policy-decision/v1`、PR summary、optional `change-package/v2` | `pr-review-completeness`、required checks、policy labels、changed-file review | unresolved AI review thread は gate を block。対応不要コメントには disposition を返信する |
...
| Formal runner / model checker / proof tool | `pnpm run verify:formal`, `verify:tla`, `verify:csp`, `verify:lean` | proof/model-check result、counterexample、assumption | tool output は model/proof scope のみを支える。製品全体の証明ではない | formal summary、`assurance-summary/v1`, `claim-evidence-manifest/v1`, optional `change-package/v2` | formal summary generation、schema validation、assumption/scope review | counterexample は affected claim の blocking evidence。bounded/model assumption を記録。waiver は failed proof を `proved` にしない |
