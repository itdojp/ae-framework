...
| CI / test runner | `fixtures/agents/evidence-adapters/ci/test-runner-output.json` | check-run conclusion, command transcript, coverage note, stale-run classification | `verify-lite-run-summary`, `quality-scorecard`, `report-envelope`, `producer-normalization-summary/v1`, optional `claim-evidence-manifest/v1`, `hook-feedback/v1` | Green checks support scoped test evidence only. They do not prove high-assurance claims or replace policy decisions. |
| Formal / model checker | `fixtures/agents/evidence-adapters/formal/model-check-output.json` | model-check result, bounded search result, assumptions, counterexample state, out-of-scope list | `producer-normalization-summary/v1`, `formal-summary/v2`, `claim-evidence-manifest/v1`, `assurance-summary/v1`, optional `change-package/v2` | Formal output supports only the modeled/proved scope and assumptions. Out-of-scope production behavior remains partial or unresolved. |

The expected report-only normalizer outputs for the CI and formal fixtures are pinned in `fixtures/agents/producer-normalization-summary.ci.json` and `fixtures/agents/producer-normalization-summary.formal.json`. They are schema-validated summaries, not approvals.
...
`fixtures/agents/evidence-adapters/` 配下の fixture は raw output 例です。新しい contract ではありません。各 fixture は、adapter が populate する、または既存 generator へ要求する想定の existing artifact を記録します。各 fixture は `producer`、`source`、`rawSignals`、`changedFiles`、`commands`、`claimsMentioned`、`expectedArtifacts`、`knownGaps` を持つ共通の説明用 shape に揃えます。この shape は後続 Issue が schema を導入するまで documentation-only であり、JSON 構文確認は生成 artifact の Contract Catalog validation を代替しません。`claimsMentioned` は target artifact ごとの語彙を使います。manifest への routing は `partial`、`satisfied`、`waived`、`unresolved`、policy decision への routing は `pass`、`waived`、`report-only`、`block` を使います。同じ tree の producer 別 README は、reviewer 向けに trust boundary と expected normalized routing を要約します。
...
| CI / test runner | `fixtures/agents/evidence-adapters/ci/test-runner-output.json` | check-run conclusion、command transcript、coverage note、stale-run classification | `verify-lite-run-summary`, `quality-scorecard`, `report-envelope`, `producer-normalization-summary/v1`, optional `claim-evidence-manifest/v1`, `hook-feedback/v1` | green check は scoped test evidence を支えるだけであり、high-assurance claim の proof や policy decision の代替ではない。 |
| Formal / model checker | `fixtures/agents/evidence-adapters/formal/model-check-output.json` | model-check result、bounded search result、assumption、counterexample state、out-of-scope list | `producer-normalization-summary/v1`, `formal-summary/v2`, `claim-evidence-manifest/v1`, `assurance-summary/v1`, optional `change-package/v2` | formal output は modeled/proved scope と assumption の範囲だけを支える。out-of-scope production behavior は partial / unresolved のまま残す。 |

CI / formal fixture の expected report-only normalizer output は `fixtures/agents/producer-normalization-summary.ci.json` と `fixtures/agents/producer-normalization-summary.formal.json` に固定しています。これは schema validated summary であり、approval ではありません。
