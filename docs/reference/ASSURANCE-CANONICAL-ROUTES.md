...
| --- | --- | --- |
| Repoint `quality:scorecard` to v1 | The v1 producer needs required inputs, while legacy workflow usage can run without explicit artifact paths. | A wrapper or workflow migration test that supplies `verify-lite-run-summary` and `report-envelope` to v1. |
| Remove or demote `formal/summary.json` in PR summary fallback order | `render-pr-summary.mjs` currently reads it before the hermetic aggregate, and `pr-ci-status-comment.yml` still reads it directly. | Renderer and workflow tests showing the formal line is populated from current canonical artifacts without changing existing summaries unexpectedly. |
...
| Quality scorecard | `quality-scorecard/v1`: `schema/quality-scorecard.schema.json`, `scripts/quality/build-quality-scorecard.mjs`, `pnpm run quality:scorecard:v1`, `pnpm run quality:scorecard:validate` | legacy `pnpm run quality:scorecard` / `scripts/quality-scorecard-generator.js` | 新しい PR / release evidence は v1 を使う。legacy command は workflow consumer が test 付きで移行されるまで互換 diagnostic route として残す。 |
| Formal evidence status | `artifacts/formal/formal-summary-v2.json` と dual-write 期間の `artifacts/formal/formal-summary-v1.json`; aggregate view は `artifacts/hermetic-reports/formal/summary.json` | legacy `formal/summary.json` | `formal/summary.json` は compatibility input としてのみ扱う。現行 PR summary rendering は hermetic aggregate より先にこれを読むため、新規 producer は formal summary v1/v2 または hermetic aggregate を出す。 |
...
| --- | --- | --- |
| `quality:scorecard` を v1 に向ける | v1 producer には必須入力があり、legacy workflow usage は明示 artifact path なしで実行できる。 | `verify-lite-run-summary` と `report-envelope` を v1 に渡す wrapper または workflow migration test。 |
| PR summary fallback order から `formal/summary.json` を外す、または後順位へ移す | `render-pr-summary.mjs` は現在 hermetic aggregate より先にこれを読み、`pr-ci-status-comment.yml` も直接参照している。 | 既存 summary を予期せず変えずに current canonical artifact から formal line が埋まることを示す renderer / workflow tests。 |
