...
| `artifacts/formal/formal-summary-v1.json` | `schema/formal-summary-v1.schema.json` | `scripts/formal/generate-formal-summary-v1.mjs` (`--out`), `.github/workflows/verify-lite.yml`, `.github/workflows/formal-aggregate.yml`; the generator preserves complete `provenance=runner-reported` execution evidence but never fabricates missing tool/version/input/result/scope/assumptions | `scripts/ci/validate-formal-summary-v1.mjs`, `scripts/ci/validate-artifacts-ajv.mjs`, `scripts/ci/generate-run-manifest.mjs`, `scripts/assurance/aggregate-lanes.mjs`; legacy schema-valid results without complete runner-reported execution evidence remain readable but are not eligible for model/proof lane promotion |
| `artifacts/formal/formal-summary-v2.json` | `schema/formal-summary-v2.schema.json` | `scripts/formal/generate-formal-summary-v1.mjs` (`--out-v2`), `.github/workflows/verify-lite.yml`, `.github/workflows/formal-aggregate.yml`; the generator preserves complete `provenance=runner-reported` execution evidence and omits incomplete/backfilled evidence | `scripts/ci/validate-formal-summary-v2.mjs`, `scripts/ci/validate-artifacts-ajv.mjs`, `scripts/ci/generate-run-manifest.mjs`, `scripts/assurance/aggregate-lanes.mjs`; v2 identity and complete runner-reported execution evidence are required for model/proof lane promotion |
| `artifacts/codex/model-check.json` | `schema/model-check-execution-v1.schema.json` | `scripts/verify/run-model-checks.mjs` via `pnpm run model-check` / `pnpm run verify:model`, `.github/workflows/verify.yml`; TLC/Alloy attempts use `executionStatus=executed|timeout|tool-error`, and completed checker failures remain `executed` with `outcome=fail` | Verify PR summary and `enforce-formal`; `status=not-run|tool-error` with `ok=null` is explicit non-success, only completed `executionStatus=executed` results are counted, and every attempt preserves tool/version (or `unknown` plus artifact SHA-256), input, result, scope, and assumptions |
...
| `artifacts/formal/formal-summary-v1.json` | `schema/formal-summary-v1.schema.json` | `scripts/formal/generate-formal-summary-v1.mjs` (`--out`), `.github/workflows/verify-lite.yml`, `.github/workflows/formal-aggregate.yml`。generator は complete な `provenance=runner-reported` execution evidence だけを保持し、欠落した tool/version/input/result/scope/assumptions を生成しない | `scripts/ci/validate-formal-summary-v1.mjs`, `scripts/ci/validate-artifacts-ajv.mjs`, `scripts/ci/generate-run-manifest.mjs`, `scripts/assurance/aggregate-lanes.mjs`。complete な runner-reported execution evidence のない legacy result は readable だが model/proof lane へ昇格しない |
| `artifacts/formal/formal-summary-v2.json` | `schema/formal-summary-v2.schema.json` | `scripts/formal/generate-formal-summary-v1.mjs` (`--out-v2`), `.github/workflows/verify-lite.yml`, `.github/workflows/formal-aggregate.yml`。generator は complete な `provenance=runner-reported` execution evidence を保持し、不完全または backfilled evidence は省略する | `scripts/ci/validate-formal-summary-v2.mjs`, `scripts/ci/validate-artifacts-ajv.mjs`, `scripts/ci/generate-run-manifest.mjs`, `scripts/assurance/aggregate-lanes.mjs`。model/proof lane への昇格には v2 identity と complete な runner-reported execution evidence が必要 |
| `artifacts/codex/model-check.json` | `schema/model-check-execution-v1.schema.json` | `scripts/verify/run-model-checks.mjs` を `pnpm run model-check` / `pnpm run verify:model` または `.github/workflows/verify.yml` から実行。TLC/Alloy attempt は `executionStatus=executed|timeout|tool-error` を使い、checker が完了して検証失敗した場合は `executed` / `outcome=fail` とする | Verify PR summary と `enforce-formal`。`status=not-run|tool-error` / `ok=null` は明示的な non-success であり、完了した `executionStatus=executed` result だけを集計する。各 attempt は tool/version（または `unknown` と artifact SHA-256）、input、result、scope、assumptions を保持する |
| `artifacts/reference/benchmarks/bench.json` | `schema/benchmark-report.schema.json` | `src/commands/bench/run.ts` (`ae bench`) | `tests/scripts/benchmark-report-schema.test.ts`, `scripts/quality/bench-compare.mjs` |
