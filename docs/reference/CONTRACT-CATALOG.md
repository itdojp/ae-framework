...
- `schema/completion-audit-report.schema.json`
...
- `schema/formal-runner-output-v1.schema.json`
- `schema/model-check-execution-v1.schema.json`
...
- `schema/exec-plan.v2.schema.json`
...
| --- | --- | --- | --- |
| `artifacts/reference/benchmarks/assurance-gate-cache-comparison-*.json` | `schema/assurance-gate-cache-comparison.schema.json` (`assurance-gate-cache-comparison/v1`) | reviewed copies of the completed #3641 experiment artifact generated by `scripts/actions/summarize-assurance-gate-cache-comparison.mjs` | `tests/scripts/assurance-gate-cache-comparison.test.ts`, `docs/operate/ASSURANCE-GATE-STARTUP-DECISION.md`; historical rollback evidence, not an active action feature or productivity claim |
...
| `docs/operate/publication-evidence.json` | `schema/publication-evidence-v1.schema.json` (`publication-evidence/v1`) | release owner or repository admin updates verified public evidence in a reviewed PR; Marketplace runtime smoke is produced by the allowlisted external consumer workflow and remains non-authoritative until owner verification | `scripts/release/validate-publication-evidence.mjs`, `scripts/release/publication-evidence-contract.mjs`, `pnpm run publication:evidence:validate`, `scripts/ci/run-verify-lite-local.sh`, release/QUICKSTART claim boundaries; default validation is deterministic and offline and checks public URL allowlists, pass/block artifact evidence, action-ref/tag consistency, and immutable/moving resolved-commit parity |
...
| `artifacts/assurance/assurance-summary.json` | `schema/assurance-summary.schema.json` | `scripts/assurance/aggregate-lanes.mjs`, `.github/workflows/verify-lite.yml`; optional assurance inputs surface lane evidence plus `reviewSurface`; raw pseudo model-check JSON and formal results labelled `draft` / `synthetic` / `unverified` / `test-only` are rejected from model/proof lanes | `scripts/ci/validate-assurance-summary.mjs`, `scripts/ci/validate-artifacts-ajv.mjs`, `scripts/ci/validate-json.mjs`, `scripts/ci/enforce-assurance-summary.mjs`, `scripts/quality/build-quality-scorecard.mjs`, `scripts/agents/build-hook-feedback.mjs`, `scripts/agents/create-handoff.mjs`, `scripts/summary/render-pr-summary.mjs`, `.github/workflows/pr-ci-status-comment.yml`; eligible formal results require formal-summary v1/v2 plus adapter-verified tool/version/input/result/scope/assumption `executionEvidence`; claim eligibility is derived from the reviewed producer-bound `verificationKind`, so Kani `presence`, Lean `build`, and CSP `typecheck` records cannot promote model/proof lanes; raw runner-reported and direct conformance-report inputs remain report-only, backfilled evidence is rejected, and `reviewSurface` does not approve merge by itself |
...
| `artifacts/formal/formal-summary-v1.json` | `schema/formal-summary-v1.schema.json` + `schema/formal-runner-output-v1.schema.json` + `schema/formal-execution-evidence-v1.schema.json` | `scripts/formal/generate-formal-summary-v1.mjs` (`--out`), `.github/workflows/verify-lite.yml`, `.github/workflows/formal-aggregate.yml`; actual runners emit a closed `runnerResult` envelope, and the generator cross-binds its reviewed producer and runner-reported evidence before emitting adapter-verified evidence | `scripts/ci/validate-formal-summary-v1.mjs`, `scripts/ci/validate-artifacts-ajv.mjs`, `scripts/ci/generate-run-manifest.mjs`, `scripts/assurance/aggregate-lanes.mjs`; missing/invalid/self-asserted evidence is downgraded, and unknown or mismatched tool-version provenance remains an ineligible evidence gap |
| `artifacts/formal/formal-summary-v2.json` | `schema/formal-summary-v2.schema.json` + `schema/formal-execution-evidence-v1.schema.json` | `scripts/formal/generate-formal-summary-v1.mjs` (`--out-v2`), `.github/workflows/verify-lite.yml`, `.github/workflows/formal-aggregate.yml`; v2 adds `contractId=formal-summary.v2` to the same closed adapter output | `scripts/ci/validate-formal-summary-v2.mjs`, `scripts/ci/validate-artifacts-ajv.mjs`, `scripts/ci/generate-run-manifest.mjs`, `scripts/assurance/aggregate-lanes.mjs`; whole-document Schema validation, exact summary/runner producer and `verificationKind` bindings, successful semantic execution, materialized log, and verified version provenance are required for model/proof promotion; SMT/SPIN exit code alone is ineligible |
| `artifacts/codex/model-check.json` | `schema/model-check-execution-v1.schema.json` + `schema/formal-execution-evidence-v1.schema.json` | `scripts/verify/run-model-checks.mjs` via `pnpm run model-check` / `pnpm run verify:model`, `.github/workflows/verify.yml`; TLC/Alloy attempts use `executionStatus=executed|timeout|tool-error`, completed checker failures remain `executed` with `result.status=failed`, and input coverage is explicit in `detectedInputs` / `executedInputs` / `skippedInputs` | Verify PR summary and `enforce-formal`; report-only is the default, while enforcement requires a valid reviewed `verificationKind=model-check` producer contract, at least one execution, all detected inputs executed and passing, zero errors/timeouts/unapproved skips, verified tool versions, and repository-relative regular non-symlink referenced logs materialized inside the uploaded artifact root |
...
| `artifacts/container-security/container-runtime-diagnostic.json`; `fixtures/container-runtime/*.container-runtime-diagnostic.json` | `schema/container-runtime-diagnostic-v1.schema.json` (`container-runtime-diagnostic/v1`) | `scripts/ci/container-runtime-preflight.mjs`, `.github/workflows/security.yml` Container Security job; runtime inventory and minimal smoke are observed, while committed fixtures are test-only | exact-head Container Security review, `scripts/ci/validate-json.mjs`, focused contract/workflow tests, and the uploaded `container-runtime-diagnostic` artifact; closed classifications preserve runtime/build/export/scan/SARIF failures, explicit runtime binding, bounded system-path/version evidence, and do not turn missing artifacts into success |
...
| `artifacts/metrics/req2run-metrics.json`; `fixtures/metrics/req2run/expected.req2run-metrics.json` | `schema/req2run-metrics.schema.json` (`req2run-metrics/v1`) | `scripts/metrics/collect-req2run-metrics.mjs` (`pnpm run metrics:req2run`), fixture input under `fixtures/metrics/req2run/` | `scripts/ci/validate-json.mjs`, `tests/scripts/req2run-metrics.test.ts`, `docs/product/REQ2RUN-METRICS.md`; report-only adoption-readiness evidence for requirement-to-runnable flow, deterministic replay, task/evidence coverage, and manual-intervention counts; no policy-gate block condition or agent-vendor ranking is added |
| `fixtures/metrics/evidence-sprint/pilot-measurement-example.json` | `schema/evidence-sprint-measurement-report.schema.json` (`evidence-sprint-measurement-report/v1`) | Manual/offline Evidence Sprint measurement fixture and `docs/templates/evidence-sprint-pilot-report.md` | `scripts/ci/validate-json.mjs`, `docs/product/DOGFOODING-PILOT-MEASUREMENT-PROTOCOL.md`; report-only dogfooding/pilot metrics for time-to-first-runnable-verification, evidence coverage, missing evidence, unresolved claims, review rework, deterministic replay, manual interventions, and completion-audit discrepancies; no policy-gate block condition, approval authority, public speed/safety claim, or agent-vendor ranking is added |
| `artifacts/audit/completion-audit-report.json`; `fixtures/audit/completion/expected/completion-audit-report.json` | `schema/completion-audit-report.schema.json` (`completion-audit-report/v1`) | `scripts/audit/render-completion-audit.mjs` (`pnpm run audit:completion`), fixture input under `fixtures/audit/completion/` | `scripts/ci/validate-json.mjs`, `tests/scripts/completion-audit.test.ts`, `docs/ci/completion-audit-report.md`; report-only PR closeout evidence that separates required merge checks from advisory workflow findings, skipped runs, stale runs, local verification, and audit notes; it does not approve, merge, waive findings, or add a policy-gate block condition |
...
| `artifacts/plan/exec-plan.v2.json`; `artifacts/plan/exec-plan.v2.md` | `schema/exec-plan.v2.schema.json` for JSON; Markdown is non-authoritative rendering | `scripts/exec-plan/validate-v2.mjs` (`pnpm run exec-plan:v2:validate`), fixtures under `fixtures/exec-plan/` | `scripts/ci/validate-json.mjs`, `docs/reference/EXEC-PLAN-V2.md`, future PR/Issue review surfaces; preview/report-only handoff from Issue intent to Context Pack refs, validation commands, evidence requirements, risk policy, stop rules, rollback, and output artifacts; does not replace `plan-artifact/v1` policy-gate enforcement in this issue |
...
| `fixtures/loop/*.loop-policy.json` | `schema/loop-policy.schema.json` | manual loop policy authoring and optional `scripts/loop/run-report-only.mjs --policy`; default policy is embedded for fixture-only runs | `scripts/ci/validate-json.mjs`, `tests/scripts/loop-runner.test.ts`, `docs/automation/LOOP-ENGINEERING-SAFETY.md`; report-only safety budgets, command/path allow/deny controls, evidence requirements, high-risk approval thresholds, repeated-failure stop rules, and redaction posture; no blocking policy gate is added |
| `examples/loop-engineering/*/loop-input.json` | `schema/loop-run-input.schema.json` | manual fixture authoring from Issue, Context Pack refs, and ExecPlan v2 paths | `scripts/loop/run-report-only.mjs`, `scripts/ci/validate-json.mjs`, `tests/scripts/loop-runner.test.ts`; report-only input contract for safe plan-observe-verify iterations, fixture validation commands, allowed safety flags, and explicit decisions |
| `artifacts/loop/loop-run-summary.json`; `fixtures/loop/*.loop-run-summary.json` | `schema/loop-run-summary.schema.json` | `scripts/loop/run-report-only.mjs` (`pnpm run loop:run-report-only`), fixture inputs under `examples/loop-engineering/` | `scripts/ci/validate-json.mjs`, `tests/scripts/loop-runner.test.ts`, `docs/integrations/LOOP-ENGINEERING.md`, `docs/automation/LOOP-ENGINEERING-SAFETY.md`; report-only loop evidence for Issue/ExecPlan-driven plan-observe-verify iterations, explicit stop reasons, validation commands, observed evidence, findings, observability metrics, replay hashes, and human-review-ready next actions; no merge/review approval/push authority is added |
...
- FormalAgent/Codex scaffold generation is not formal execution. Draft/synthetic/unverified/test-only artifacts and legacy formal summaries without complete execution evidence remain report-only and cannot create model/proof evidence.
...
| `artifacts/assurance/assurance-summary.json` | `schema/assurance-summary.schema.json` | `scripts/assurance/aggregate-lanes.mjs`, `.github/workflows/verify-lite.yml`。optional assurance input から lane evidence と PR review 用 `reviewSurface` を表示する。raw pseudo model-check JSON と `draft` / `synthetic` / `unverified` / `test-only` formal result は model/proof lane から除外する | `scripts/ci/validate-assurance-summary.mjs`, `scripts/ci/validate-artifacts-ajv.mjs`, `scripts/ci/validate-json.mjs`, `scripts/ci/enforce-assurance-summary.mjs`, `scripts/quality/build-quality-scorecard.mjs`, `scripts/agents/build-hook-feedback.mjs`, `scripts/agents/create-handoff.mjs`, `scripts/summary/render-pr-summary.mjs`, `.github/workflows/pr-ci-status-comment.yml`。eligible formal result は formal-summary v1/v2 と adapter-verified tool/version/input/result/scope/assumption の `executionEvidence` が必要。claim eligibility は review 済み producer に bind した `verificationKind` から導出し、Kani `presence`、Lean `build`、CSP `typecheck` は model/proof lane へ昇格しない。raw runner-reported と direct conformance-report input は report-only とし、backfill evidence は除外し、`reviewSurface` は report-only であり、それ自体では merge 承認にならない |
...
| `artifacts/formal/formal-summary-v1.json` | `schema/formal-summary-v1.schema.json` + `schema/formal-runner-output-v1.schema.json` + `schema/formal-execution-evidence-v1.schema.json` | `scripts/formal/generate-formal-summary-v1.mjs` (`--out`), `.github/workflows/verify-lite.yml`, `.github/workflows/formal-aggregate.yml`。actual runner は closed `runnerResult` envelope を生成し、generator は review 済み producer と runner-reported evidence を cross-bind してから adapter-verified evidence を出力する | `scripts/ci/validate-formal-summary-v1.mjs`, `scripts/ci/validate-artifacts-ajv.mjs`, `scripts/ci/generate-run-manifest.mjs`, `scripts/assurance/aggregate-lanes.mjs`。missing/invalid/self-asserted evidence は downgrade し、unknown または mismatch の tool-version provenance は ineligible Evidence gap として保持する |
| `artifacts/formal/formal-summary-v2.json` | `schema/formal-summary-v2.schema.json` + `schema/formal-execution-evidence-v1.schema.json` | `scripts/formal/generate-formal-summary-v1.mjs` (`--out-v2`), `.github/workflows/verify-lite.yml`, `.github/workflows/formal-aggregate.yml`。v2 は同じ closed adapter output に `contractId=formal-summary.v2` を追加する | `scripts/ci/validate-formal-summary-v2.mjs`, `scripts/ci/validate-artifacts-ajv.mjs`, `scripts/ci/generate-run-manifest.mjs`, `scripts/assurance/aggregate-lanes.mjs`。model/proof 昇格には文書全体の Schema validation、summary/runner producer と `verificationKind` binding の完全一致、semantic execution 成功、materialized log、verified version provenance を要求し、SMT/SPIN の exit code だけでは eligibility を得られない |
| `artifacts/codex/model-check.json` | `schema/model-check-execution-v1.schema.json` + `schema/formal-execution-evidence-v1.schema.json` | `scripts/verify/run-model-checks.mjs` を `pnpm run model-check` / `pnpm run verify:model` または `.github/workflows/verify.yml` から実行。TLC/Alloy attempt は `executionStatus=executed|timeout|tool-error`、checker failure は `executed` / `result.status=failed` とし、`detectedInputs` / `executedInputs` / `skippedInputs` で input coverage を明示する | Verify PR summary と `enforce-formal`。既定は report-only とし、enforcement は valid な review 済み `verificationKind=model-check` producer contract、1件以上の execution、検出 input 全件の実行・成功、error/timeout/未承認 skip 0件、verified tool version、uploaded artifact root 内に materialize された repository-relative / regular / non-symlink の全参照 log を要求する |
| `artifacts/reference/benchmarks/bench.json` | `schema/benchmark-report.schema.json` | `src/commands/bench/run.ts` (`ae bench`) | `tests/scripts/benchmark-report-schema.test.ts`, `scripts/quality/bench-compare.mjs` |
...
| `artifacts/container-security/container-runtime-diagnostic.json`; `fixtures/container-runtime/*.container-runtime-diagnostic.json` | `schema/container-runtime-diagnostic-v1.schema.json`（`container-runtime-diagnostic/v1`） | `scripts/ci/container-runtime-preflight.mjs`と`.github/workflows/security.yml`のContainer Security job。runtime inventory／minimal smokeはobserved evidenceで、commit済みfixtureはtest-only | exact-head Container Security review、`scripts/ci/validate-json.mjs`、focused contract/workflow tests、upload済み`container-runtime-diagnostic` artifact。closed classificationとshared status semanticsはruntime／build／export／scan／SARIF failureを非変換で保持し、explicit runtimeをsystem path/versionへbindする。archive／SARIF／diagnostic readは全path componentのsymlinkとoversized structured artifactをfail closedで拒否する。runtime-readyなpreflight reportは`pipelineComplete=false`であり、12個のreview済みcheck（`image-user`を`archive-export`の必須前提に含む）が一意にpassしてexplicit finalizationが完了した`pipelineComplete=true`だけをcomplete Container Security Evidenceとして扱う |
| `artifacts/agents/hook-feedback.json` | `schema/hook-feedback.schema.json` | `scripts/agents/build-hook-feedback.mjs`, `.github/workflows/pr-ci-status-comment.yml` | `scripts/ci/validate-artifacts-ajv.mjs`, `scripts/agents/create-handoff.mjs`, `docs/agents/hook-feedback.md`, Claude Code / Codex continuation consumers |
...
| `fixtures/metrics/evidence-sprint/pilot-measurement-example.json` | `schema/evidence-sprint-measurement-report.schema.json`（`evidence-sprint-measurement-report/v1`） | manual/offline Evidence Sprint measurement fixture と `docs/templates/evidence-sprint-pilot-report.md` | `scripts/ci/validate-json.mjs`, `docs/product/DOGFOODING-PILOT-MEASUREMENT-PROTOCOL.md`。time-to-first-runnable-verification、evidence coverage、missing evidence、unresolved claim、review rework、deterministic replay、manual intervention、completion-audit discrepancy を report-only dogfooding/pilot metrics として記録する。policy-gate block 条件、承認権限、public speed/safety claim、agent-vendor ranking は追加しない |
| `artifacts/audit/completion-audit-report.json`; `fixtures/audit/completion/expected/completion-audit-report.json` | `schema/completion-audit-report.schema.json`（`completion-audit-report/v1`） | `scripts/audit/render-completion-audit.mjs`（`pnpm run audit:completion`）、fixture input は `fixtures/audit/completion/` 配下 | `scripts/ci/validate-json.mjs`, `tests/scripts/completion-audit.test.ts`, `docs/ci/completion-audit-report.md`。required merge checks、advisory workflow findings、skipped runs、stale runs、local verification、audit notes を分離する report-only PR closeout evidence。承認・マージ・waiver・policy-gate block 条件追加は行わない |
...
| `artifacts/quality/variance-report.json` | `schema/variance-report.schema.json` | `scripts/quality/compare-judgment-artifacts.mjs`, `pnpm run quality:variance`。fixture input は `fixtures/variance/` 配下 | `scripts/ci/validate-json.mjs`, `docs/quality/VARIANCE-REDUCTION-POLICY.md`。normalized judgment artifact、input hash、許容volatile差分、drift finding を report-only で比較し、通常PRの blocking gate は追加しない |
| `fixtures/loop/*.loop-policy.json` | `schema/loop-policy.schema.json` | manual loop policy authoring と任意の `scripts/loop/run-report-only.mjs --policy`。fixture-only run には embedded default policy を使う | `scripts/ci/validate-json.mjs`, `tests/scripts/loop-runner.test.ts`, `docs/automation/LOOP-ENGINEERING-SAFETY.md`。安全budget、command/path allow/deny、evidence requirement、high-risk approval threshold、repeated-failure stop rule、redaction姿勢を report-only で固定する。blocking policy gate は追加しない |
| `examples/loop-engineering/*/loop-input.json` | `schema/loop-run-input.schema.json` | Issue、Context Pack refs、ExecPlan v2 path から manual に作成する fixture | `scripts/loop/run-report-only.mjs`, `scripts/ci/validate-json.mjs`, `tests/scripts/loop-runner.test.ts`。安全な plan-observe-verify iteration、fixture validation command、許可済み safety flag、明示的 decision を表す report-only input contract |
| `artifacts/loop/loop-run-summary.json`; `fixtures/loop/*.loop-run-summary.json` | `schema/loop-run-summary.schema.json` | `scripts/loop/run-report-only.mjs`（`pnpm run loop:run-report-only`）、`examples/loop-engineering/` 配下の fixture input | `scripts/ci/validate-json.mjs`, `tests/scripts/loop-runner.test.ts`, `docs/integrations/LOOP-ENGINEERING.md`, `docs/automation/LOOP-ENGINEERING-SAFETY.md`。Issue / ExecPlan 起点の plan-observe-verify iteration、明示的 stop reason、validation command、observed evidence、finding、observability metrics、replay hash、human review 向け next action を report-only evidence として記録する。merge / review approval / push 権限は追加しない |
| `artifacts/reference/legal/license-scope-audit.json` | `schema/license-scope-audit.schema.json` | `scripts/legal/inventory-license-scope.mjs` | `scripts/ci/validate-json.mjs`, `docs/project/LICENSE-MIGRATION-AUDIT.md`, downstream legal audit builders |
| `artifacts/reference/legal/conditional-asset-audit.json` | `schema/conditional-asset-audit.schema.json` | `scripts/legal/inventory-conditional-assets.mjs` | `scripts/ci/validate-json.mjs`, `docs/project/CONDITIONAL-ASSET-PROVENANCE-AUDIT.md`, downstream legal audit builders |
| `artifacts/reference/legal/notice-readiness-audit.json` | `schema/notice-readiness-audit.schema.json` | `scripts/legal/build-notice-readiness.mjs` | `scripts/ci/validate-json.mjs`, `docs/project/NOTICE-READINESS-AUDIT.md`, operator/legal review before `NOTICE` cutover |
| `artifacts/reference/legal/contributor-license-readiness-audit.json` | `schema/contributor-license-readiness-audit.schema.json` | `scripts/legal/build-contributor-license-readiness.mjs` | `scripts/ci/validate-json.mjs`, `docs/project/CONTRIBUTOR-LICENSE-READINESS.md`, human/legal contributor review before relicensing |
...
- FormalAgent/Codex scaffold generation は formal execution ではない。draft/synthetic/unverified/test-only artifact と complete execution evidence を持たない legacy formal summary は report-only のままにし、model/proof evidence を生成しない。
