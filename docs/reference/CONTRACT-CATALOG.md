...
- `claim-level-summary/v1` is the PR/release projection. It summarizes supported, tested, model-checked, proved, runtime-mitigated, waived, unresolved, failed, and not-applicable counts without changing the source manifest contract.
...
- `claim-level-summary/v1` は PR / release projection です。source manifest contract を変更せず、supported、tested、model-checked、proved、runtime-mitigated、waived、unresolved、failed、not-applicable の count を要約します。
