...
| Producer class | Raw signal | Normalized ae-framework artifact | Schema / validator | Primary consumer | Enforcement default | Gap IDs |
| --- | --- | --- | --- | --- | --- | --- |
...
| GitHub Copilot PR review / agent | review body, inline comments, suggestions, thread state, PR diff | `policy-decision/v1`, optional `change-package/v2`, optional `hook-feedback/v1` | Copilot Review Gate (`scripts/ci/copilot-review-gate.mjs`, `.github/workflows/copilot-review-gate.yml`) for unresolved AI threads; `schema/policy-decision-v1.schema.json` when policy records a decision | reviewer, `policy-gate`, PR summary | unresolved AI review threads can block; generated review text is source evidence, not judgment by itself | ACP-GAP-001, ACP-GAP-003, ACP-GAP-004 |
| Human maintainer | approval, rationale, manual waiver, merge decision, command evidence | `temporary-override/v1`, `policy-decision/v1`, `claim-evidence-manifest/v1`, `change-package/v2` | `schema/temporary-override-v1.schema.json`, `schema/policy-decision-v1.schema.json`, `schema/claim-evidence-manifest.schema.json`, reviewer policy / branch protection | `policy-gate`, release reviewer, audit trail | only traceable waiver metadata can override; unresolved risk stays visible | ACP-GAP-001 |
...
| GitHub Copilot PR review / agent | review body、inline comment、suggestion、thread state、PR diff | `policy-decision/v1`、optional `change-package/v2`、optional `hook-feedback/v1` | unresolved AI thread は Copilot Review Gate（`scripts/ci/copilot-review-gate.mjs`, `.github/workflows/copilot-review-gate.yml`）、policy decision は `schema/policy-decision-v1.schema.json` | reviewer、`policy-gate`、PR summary | unresolved AI review thread は block 可能。生成review本文は source evidence であり judgment ではない | ACP-GAP-001, ACP-GAP-003, ACP-GAP-004 |
| Human maintainer | approval、rationale、manual waiver、merge decision、command evidence | `temporary-override/v1`, `policy-decision/v1`, `claim-evidence-manifest/v1`, `change-package/v2` | `temporary-override` / `policy-decision` / manifest validation、reviewer policy、branch protection | `policy-gate`、release reviewer、audit trail | traceable waiver metadata がある場合だけ override 可能。unresolved risk は残す | ACP-GAP-001 |
