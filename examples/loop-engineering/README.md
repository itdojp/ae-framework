...
The input fixtures validate against `schema/loop-run-input.schema.json`; generated summaries validate against `schema/loop-run-summary.schema.json`.
...
input fixture は `schema/loop-run-input.schema.json`、生成 summary は `schema/loop-run-summary.schema.json` で検証されます。
