...
describe('PBT: TokenBucket tiny-interval alt-pattern-21 (fast)', () => {
  it(
    formatGWT('tiny interval varied waits', 'apply waits [i/3, i/2, 2i, 1]', 'tokens within [0,max]'),
    async () => {
      await fc.assert(
        fc.asyncProperty(
          ...
          async (maxTokens, per) => {
            ...
            const waits = [Math.max(1, Math.floor(interval/3)), Math.max(1, Math.floor(interval/2)), 2*interval, 1];
