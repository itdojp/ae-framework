...
function makeRoot() {
  ...
  mkdirSync(path.join(rootDir, 'docs', 'articles', 'zenn'), { recursive: true });
  mkdirSync(path.join(rootDir, 'docs', 'benchmark'), { recursive: true });
  mkdirSync(path.join(rootDir, 'docs', 'ci'), { recursive: true });
  mkdirSync(path.join(rootDir, 'docs', 'contributing'), { recursive: true });
  mkdirSync(path.join(rootDir, 'docs', 'ddd'), { recursive: true });
  mkdirSync(path.join(rootDir, 'docs', 'development'), { recursive: true });
  mkdirSync(path.join(rootDir, 'docs', 'examples'), { recursive: true });
  ...
  mkdirSync(path.join(rootDir, 'docs', 'infra'), { recursive: true });
  ...
  mkdirSync(path.join(rootDir, 'docs', 'proposals'), { recursive: true });
...
describe('check-doc-governance', () => {
  ...
  it('governs ddd and examples docs files', () => {
    const rootDir = makeRoot();
  ...
  it('governs articles and proposals docs files', () => {
    const rootDir = makeRoot();
  ...
  it('governs benchmark docs files', () => {
    const rootDir = makeRoot();
  ...
  it('governs docs/legacy files recursively without narrative warnings', () => {
    const rootDir = makeRoot();
    ...
    writeMarkdown(rootDir, 'docs/legacy/QUICK_START.md', [
      '---',
      'docRole: narrative',
      "lastVerified: '2026-03-12'",
      '---',
      ...
      '# Quick Start',
      '',
      'This archived guide must preserve its original wording.',
      '',
    ].join('\n'));
    writeMarkdown(rootDir, 'docs/legacy/agents/verify-agent.md', [
      '---',
      'docRole: narrative',
      "lastVerified: '2026-03-12'",
      '---',
