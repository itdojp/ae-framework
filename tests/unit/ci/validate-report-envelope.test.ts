...
const legacySchemaPath = join(process.cwd(), 'schema/report-envelope.schema.json');
...
describe('validate-report-envelope CLI', () => {
  ...
  it('supports dual validation with legacy projection', async () => {
    const summaryPath = join(workdir, 'summary.json');
    const envelopePath = join(workdir, 'envelope.json');

    const summary = {
      schemaVersion: '1.0.0',
      payload: 'dual-validate'
    };
    ...
    const validateResult = spawnSync(
      process.execPath,
      [validateScript, envelopePath, schemaPath, '--dual', `--legacy-schema=${legacySchemaPath}`],
      { cwd: workdir },
    );

    expect(validateResult.status).toBe(0);
    expect(validateResult.stderr.toString()).toBe('');
    expect(validateResult.stdout.toString()).toContain('legacy envelope projection validated');
  });
