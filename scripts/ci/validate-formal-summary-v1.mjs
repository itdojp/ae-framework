...
if (fs.existsSync(executionEvidenceSchemaPath)) {
  try {
    ...
    console.error(`[formal-summary/v1] failed to read execution evidence schema ${executionEvidenceSchemaPath}:`, error);
    process.exit(1);
  }
} else if (JSON.stringify(schema).includes('formal-execution-evidence-v1.schema.json')) {
  console.error(`[formal-summary/v1] execution evidence schema not found at ${executionEvidenceSchemaPath}`);
  process.exit(1);
}
