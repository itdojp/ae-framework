...
const DEMO_NAME = 'scope-drift-demo';
...
const DEFAULT_PR_NUMBER = 3511;
const EXAMPLE_ROOT = 'examples/assurance-control-plane/scope-drift-demo';
const PRODUCER_INPUT = `${EXAMPLE_ROOT}/producer-output/codex-cli-scope-drift-output.json`;
const ASSURANCE_PROFILE = `${EXAMPLE_ROOT}/assurance-profile.json`;
const BOUNDARY_SUMMARY_FIXTURE = `${EXAMPLE_ROOT}/expected/boundary-map-summary.json`;
...
function demoPaths(outputRoot) {
  ...
  return {
    ...
    boundaryReportJson: path.join(root, 'context-pack', DEMO_NAME, 'context-pack-boundary-map-report.json'),
    boundaryReportMd: path.join(root, 'context-pack', DEMO_NAME, 'context-pack-boundary-map-report.md'),
...
function buildBoundaryReport(boundarySummary) {
  return {
    schemaVersion: 'context-pack-boundary-map-report/v1',
    generatedAt: boundarySummary.generatedAt,
    mapPath: boundarySummary.source.mapPath,
    schemaPath: boundarySummary.source.schemaPath,
    contextPackSources: ['spec/context-pack/**/*.{yml,yaml,json}'],
    scannedContextPackFiles: boundarySummary.counts.scannedContextPackFiles,
    skippedAuxiliaryFiles: boundarySummary.counts.skippedAuxiliaryFiles,
    checkedSlices: boundarySummary.counts.checkedSlices,
    checkedProduces: boundarySummary.counts.checkedProduces,
    checkedConsumes: boundarySummary.counts.checkedConsumes,
    summary: {
      totalViolations: boundarySummary.counts.totalFindings,
      duplicateSliceIds: boundarySummary.counts.duplicateSliceIds,
      missingContextPackRefs: boundarySummary.counts.missingContextPackRefs,
      duplicateProducedRefs: boundarySummary.counts.duplicateProducedRefs,
      missingUpstreamSlices: boundarySummary.counts.missingUpstreamSlices,
      missingUpstreamProducers: boundarySummary.counts.missingUpstreamProducers,
      cycleViolations: boundarySummary.counts.cycleViolations,
    },
    status: boundarySummary.status === 'ok' ? 'pass' : 'review',
    violations: boundarySummary.reviewEvidence.map((entry) => ({
      type: entry.type,
...
function renderBoundaryReportMarkdown(report) {
  return [
    ...
    `- Findings: ${report.summary.totalViolations}`,
    '',
    '## Summary',
    ...
    report.violations.length
      ? report.violations.map((entry) => `- ${entry.severity}/${entry.type}: ${entry.file} — ${entry.message}`).join('\n')
...
function buildPolicySummary({ options, paths, mode, highRiskPlanArtifact }) {
  ...
  const labels = isHighRisk
    ? ['risk:high', 'enforce-assurance', 'enforce-artifacts', 'run-ci-extended']
    : ['risk:low'];
  const statusRollup = [
  ...
  const assuranceState = {
    path: toRepoRelativePath(paths.assuranceJson),
    present: true,
    ...
    claims: [
      {
        claimId: 'scope-drift-within-declared-boundary',
        result: 'report-only',
        status: 'unresolved',
        evidenceRefs: [],
        missingEvidenceRefs: ['boundary-map-drift'],
        waiverRefs: [],
        waivers: [],
      },
    ],
    warnings: ['Boundary Map drift remains report-only unless policy selects strict assurance.'],
    errors: [],
  };
...
function run(options) {
  ...
  const boundarySummary = readJson(BOUNDARY_SUMMARY_FIXTURE);
  boundarySummary.generatedAt = options.generatedAt;
  boundarySummary.source = {
    ...boundarySummary.source,
    schemaPath: 'schema/context-pack-boundary-map.schema.json',
    reportJsonPath: toRepoRelativePath(paths.boundaryReportJson),
    reportMarkdownPath: toRepoRelativePath(paths.boundaryReportMd),
  };
  const boundaryReport = buildBoundaryReport(boundarySummary);
  writeJson(paths.boundaryReportJson, boundaryReport);
  writeText(paths.boundaryReportMd, renderBoundaryReportMarkdown(boundaryReport));
  writeJson(paths.boundaryJson, boundarySummary);
  ...
  const normalPolicy = buildPolicySummary({
    options, paths, mode: 'normal', highRiskPlanArtifact,
  });
  const highRiskPolicy = buildPolicySummary({
    options, paths, mode: 'high-risk', highRiskPlanArtifact,
  });
  ...
  runNode([
    ...
    '--title', 'Scope Drift Assurance Demo Review',
    '--producer-summary', toRepoRelativePath(paths.producerJson),
  ...
  runNode([
    'scripts/assurance/render-pr-review-surface.mjs',
    '--title', 'Scope Drift Assurance Demo Review — High-Risk Strict Lane',
    '--producer-summary', toRepoRelativePath(paths.producerJson),
    '--assurance-summary', toRepoRelativePath(paths.assuranceJson),
  ...
  process.stdout.write([
    ...
    `scope_drift_finding_count: ${boundarySummary.counts.totalFindings}`,
    `producer_missing_evidence_count: ${producer.summary?.missingEvidence ?? 0}`,
    `boundary_map_status: ${assurance.reviewSurface?.boundaryMap?.status ?? 'unknown'}`,
...
try {
  ...
  console.error(`scope-drift-demo: ${message}`);
  process.exitCode = 1;
}
