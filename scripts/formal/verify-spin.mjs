...
export function runSpinVerification(argv = process.argv) {
  ...
  const spinCommand = String(process.env.AE_FORMAL_SPIN_COMMAND || 'spin').trim() || 'spin';
  const compilerCommand = String(process.env.AE_FORMAL_C_COMPILER_COMMAND || 'gcc').trim() || 'gcc';
  const configuredPanCommand = String(process.env.AE_FORMAL_PAN_COMMAND || '').trim();
  ...
  if (!fs.existsSync(absFile)) {
    ...
    output = outputFull;
  } else if (!commandExists(spinCommand)) {
    status = 'tool_not_available';
    outputFull = "SPIN not found. Install 'spin' (and gcc) to run Promela checks.";
    output = outputFull;
  } else if (!commandExists(compilerCommand)) {
    toolVersion = extractToolVersion(runCommand(spinCommand, ['-V']).output);
    ...
    try {
      ...
      const gen = runCommand(spinCommand, ['-a', tmpModel], { cwd: tmp });
      if (!gen.available || gen.status !== 0) {
        ...
        const cc = runCommand(compilerCommand, ['-O2', '-o', 'pan', 'pan.c'], { cwd: tmp });
        if (!cc.available || cc.status !== 0) {
          status = 'compile_failed';
          outputFull = [gen.output, cc.output].filter(Boolean).join('\n');
          output = clamp(outputFull);
        } else {
          const runSpec = { cmd: configuredPanCommand || './pan', args: panArgs };
