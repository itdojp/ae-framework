...
preset, trace validation, and trace-specific formal conformance summary
without adding a runtime event processor.
...
| Conformance evidence | `fixtures/evidence-sprint/event-driven-pilot/conformance-evidence.md` | present | Shows trace validation pass and selected-trace conformance summary. |
...
- [ ] Conformance evidence is tied to `samples/conformance/sample-traces.json`,
      not to the generic `configs/samples` data/rule fixture.
...
- The selected trace summary is fixture-level evidence for schema/invariant
  compatibility, not production ordering or broker correctness.
