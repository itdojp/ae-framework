...
class TestGenerationServer {
  ...
  private async handleGeneratePropertyTests(args: unknown) {
    ...
    try {
      ...
      const outDir = path.join('tests', 'property', 'generated');
      await fs.mkdir(outDir, { recursive: true });
      const fileSafe = parsed.functionName.replace(/[^A-Za-z0-9_-]/g, '_');
      const testFile = path.join(outDir, `${fileSafe}.pbt.test.ts`);
      ...
      response += `\nEmitted artifacts to ${artifactsDir} and generated test file ${testFile}.\n`;
    } catch (e) {
