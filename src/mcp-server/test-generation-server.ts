...
class TestGenerationServer {
  ...
  private async handleGeneratePropertyTests(args: unknown) {
    ...
    // Low-impact side effect: write artifacts and a generated test file
    try {
      const artifactsDir = path.join('artifacts', 'properties');
      await fs.mkdir(artifactsDir, { recursive: true });
      await fs.writeFile(
        path.join(artifactsDir, 'design.json'),
        JSON.stringify({ contract, generated: { count: testCases.length } }, null, 2)
      );
      const outDir = path.join('artifacts', 'properties', 'generated-tests');
      await fs.mkdir(outDir, { recursive: true });
      const testFile = path.join(outDir, `property-${parsed.functionName}.pbt.test.ts`);
      const header = `// Auto-generated by TestGenerationServer\nimport { describe, it } from 'vitest';\n\n`;
      const suiteName = JSON.stringify(`${parsed.functionName} property-based tests`);
      const body = `describe(${suiteName}, () => {\n${testCases
        .map((tc) => {
          const testName = JSON.stringify(String(tc.name ?? ''));
          return `  it(${testName}, async () => {\n${tc.code}\n  });`;
        })
        .join('\n')}\n});\n`;
      await fs.writeFile(testFile, header + body, 'utf-8');
      response += `\nEmitted artifacts to ${artifactsDir} and generated test proposal ${testFile}. Review before moving it under the active test tree.\n`;
    } catch (e) {
      response += `\n(Note) Failed to write artifacts or generated file: ${(e as Error).message}\n`;
    }
  ...
  private async handleGenerateBDDScenarios(args: unknown) {
    ...
    let response = `# BDD Scenarios for ${userStory.title}\n\n`;
    response += `\`\`\`gherkin\n${gherkin}\n\`\`\`\n\n`;

    // Low-impact side effect: emit feature file and artifact summary
    try {
      const featuresDir = 'features';
      await fs.mkdir(featuresDir, { recursive: true });
      const slug = userStory.title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '');
      const featurePath = path.join(featuresDir, `${slug || 'story'}.feature`);
      await fs.writeFile(featurePath, gherkin, 'utf-8');
      const bddDir = path.join('artifacts', 'bdd');
      await fs.mkdir(bddDir, { recursive: true });
      await fs.writeFile(
        path.join(bddDir, 'scenarios.json'),
        JSON.stringify({ title: userStory.title, criteriaCount: userStory.acceptanceCriteria?.length ?? 0, feature: featurePath }, null, 2)
      );
      response += `Generated feature file: ${featurePath}\nArtifacts: ${path.join(bddDir, 'scenarios.json')}\n`;
    } catch (e) {
      response += `\n(Note) Failed to write BDD artifacts: ${(e as Error).message}\n`;
    }
