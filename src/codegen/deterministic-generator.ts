...
export class DeterministicCodeGenerator {
  ...
  private async generateTypeScriptFiles(ir: AEIR, specHash: string, timestamp: string): Promise<GeneratedFile[]> {
    ...
    const apis = ir.api ?? [];
    if (apis.length > 0) {
      const apiContent = this.generateAPIInterfaces({ ...ir, api: apis });
      files.push({
...
`;
  ...
  private generateAPIInterfaces(ir: AEIR): string {
    const apis = ir.api ?? [];
    const header = `// Generated API interfaces
// DO NOT MODIFY - This file is automatically generated

`;

    if (apis.length === 0) {
      return `${header}// No API endpoints defined in specification
`;
    }

    return `${header}${apis.map(api => `
...
`;
  ...
  private generateEntityForm(entity: AEIRDomainEntity): string {
...
`;
  ...
  private generateEntityList(entity: AEIRDomainEntity): string {
...
${entity.fields.slice(0, 3).map((field) => `          <span>{item.${field.name}}</span>`).join('\n')}
...
`;
  ...
  private generateORMModel(entity: AEIRDomainEntity): string {
...
${entity.fields.filter((field) => field.name !== 'id').map((field) => `
...
`;
  ...
  private getColumnOptions(field: AEIRDomainField): string {
