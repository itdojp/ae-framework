...
## Category Breakdown
${structured.categories.map(cat => `• **${cat.name}**: ${cat.requirements.length} requirements`).join('\n')}

## Priority Distribution
- **High Priority**: ${structured.requirements.filter(r => r.priority === 'high').length}
- **Medium Priority**: ${structured.requirements.filter(r => r.priority === 'medium').length}
- **Low Priority**: ${structured.requirements.filter(r => r.priority === 'low').length}
      `.trim(),
      recommendations: [
...
${gaps.map(g => `• ${g.suggestedRequirement}`).join('\n')}
  ...
  private async processNaturalLanguageRequirements(text: string): Promise<ProcessedRequirements> {
    ...
    const structured: RequirementDocument[] = sentences.map((sentence, index) => ({
  ...
  private detectAmbiguousLanguage(requirements: RequirementDocument[]): string[] {
    ...
    return ambiguous.map(r => `Ambiguous language in: "${r.content}"`);
  }

  private generateClarificationQuestions(requirements: RequirementDocument[]): string[] {
    return requirements
      .filter(r => r.priority === 'high' && this.detectAmbiguousLanguage([r]).length > 0)
  ...
  private async analyzeRecentActivity(context: {
    ...
    const requirementFiles = context.recentFiles.filter(f => 
