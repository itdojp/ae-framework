...
${validation.issues
  ...
  .join('\n')}
      ...
      warnings: validation.issues
        ...
        .map(i => i.description),
      shouldBlockProgress: validation.issues.filter(i => i.severity === 'critical').length > 0,
    };
  }
...
${validation.storyIssues.map(issue => `• **${issue.storyId}**: ${issue.description}`).join('\n')}
      ...
      warnings: validation.blockingIssues.map(issue => issue.description),
...
${validation.criticalGaps.map(gap => `• ${gap.description} (Impact: ${gap.impact})`).join('\n')}
      ...
      warnings: validation.criticalGaps.map(gap => gap.description),
...
${validation.orphanedArtifacts.map(artifact => 
  `• **${artifact.type}**: ${artifact.name} (no traceability)`
      ...
      warnings: validation.brokenLinks.map(link => `Broken link: ${link.from} → ${link.to}`),
...
${validation.alignmentGaps.map(gap => 
  ...
  private async analyzeRecentActivity(context: {
    ...
    const hasRequirementChanges = context.recentFiles.some(f => 
      f.includes('requirement') || f.includes('spec')
    );
    
    const hasCodeChanges = context.recentFiles.some(f => 
