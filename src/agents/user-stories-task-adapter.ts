...
## Epic Breakdown
${storySet.epics.map(epic => {
  const epicStories = storySet.stories.filter(s => s.epic === epic);
  return `• **${epic}**: ${epicStories.length} stories (${epicStories.reduce((sum, s) => sum + s.storyPoints, 0)} points)`;
}).join('\n')}

## Priority Distribution
- **High Priority**: ${storySet.stories.filter(s => s.priority === 'high').length} stories
- **Medium Priority**: ${storySet.stories.filter(s => s.priority === 'medium').length} stories
- **Low Priority**: ${storySet.stories.filter(s => s.priority === 'low').length} stories

## Identified Gaps
${storySet.gaps.map(gap => `• ${gap}`).join('\n')}
      `.trim(),
      recommendations: [
...
${dependencies.risks.map(risk => 
  ...
  private async analyzeRecentActivity(context: {
    ...
    const storyFiles = context.recentFiles.filter(f => 
